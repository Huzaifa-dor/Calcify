...
function cosCalculator(val1, val2, container, box1, box2) {
    if (isNumber(val1, val2)){
            ...
            let result = y1/y2;
            result = result.toFixed(2);
            container.textContent = `The Cosine θ value of Base ${y1} and Hypotenuse ${y2} is cosθ = ${result} !`;
            box1.value = "";
            box2.value = "";
            }else{
                alert("Hypotenuse Shall be greater than perpendicular and base Sussy baka!")
...
const cosLBase = document.getElementById('cosLBase');
const cosLPerp = document.getElementById('cosLPerp');
const cosLAngC = document.getElementById('cosLAngC');
const cosLButton = document.getElementById('cosLButton');
const cosLContainer = document.getElementById('cosLContainer');
function cosLCalculator (val1, val2, val3, container, box1, box2, box3) {
    if (isNumber(val1, val2)){
        if(isNumber(val3, "1")) {
            let y1 = Number(val1);
            let y2 = Number(val2);
            let y3 = Number(val3);
            let y3R = y3*Math.PI/180;
            let result = Math.sqrt((y1*y1)+(y2*y2)-2*y1*y2*Math.cos(y3R));
            result = result.toFixed(2);
            container.textContent = `The Value of Side C with Side A ${y1} and Side B ${y2} and Angle C ${y3} is ${result}!`;
            box1.textContent = "";
            box2.textContent = "";
            box3.textContent = "";
        }else{
...
cosLButton.addEventListener('click', function() {
    let cosLB = cosLBase.value;
    let cosLP = cosLPerp.value;
    let cosLAC = cosLAngC.value;
    cosLCalculator(cosLB, cosLP, cosLAC, cosLContainer, cosLBase, cosLPerp, cosLAngC)
})